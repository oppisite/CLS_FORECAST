...
export class SalesDashboardComponent {
    ...
    quantityBarChartOptions: BarChartOptions | null = null;
    valueBarChartOptions: BarChartOptions | null = null;
    ...
    griddataMasDrug: any[] = [];
    // ตัวแปรเพิ่มเติมสำหรับ Drug Customer
    ...
    private dataSubscriptionDrugCustomer: Subscription | null = null;
        // เพิ่มตัวแปร
    ...
    public servicedrugCustomer: GridJsService;
    Detail_product: any[] = [];
    Show_List_MAS_Customer: any[] = [];
  List_MAS_Customer: any[] = [];
  filteredCustomersForDropdown: any[] = [];
  customerSearchTerm: string = '';
    ...
    constructor(private modalService: NgbModal, public service: GridJsService
        ...
        this.servicedrugCustomer = new GridJsService(decimalPipe);
        ...
        this.gridjsListDrugCustomer$ = this.servicedrugCustomer.countries$;
        this.totalDrugCustomer$ = this.servicedrugCustomer.total$;
    ...
    ngOnInit(): void {
        ...
        // Subscribe สำหรับ Drug Customer
        if (!this.dataSubscriptionDrugCustomer) {
            this.dataSubscriptionDrugCustomer = this.gridjsListDrugCustomer$.subscribe((data: any) => {
                this.griddataDrugCustomer = Object.assign([], data);
            });
            this.subscriptions.push(this.dataSubscriptionDrugCustomer);
        }
    ...
    filterDrugs(): void {
        this.Show_List_MAS_Customer = (this.List_MAS_Customer || []).filter((customer: any) => {
          return !this.facilityId || customer.CustomerTypeCode === this.facilityId;
        });
        ...
        this.filteredDrugs = (this.LIST_MAS_DRUG || []).filter((drug: any) => {
            ...
            const matchFacility = !this.facilityId || drug.FacilityId === this.facilityId;
            const matchCustomer = !this.customerId || drug.CustomerId === this.customerId;
            return matchType && matchGroup && matchFacility && matchCustomer;
        });
    }

    // อัพเดท filtered customers สำหรับ dropdown (แสดงทีละ 10 รายการ)
    updateFilteredCustomers(): void {
        if (!this.customerSearchTerm || this.customerSearchTerm.trim() === '') {
            // ถ้าไม่มี search term ให้แสดง 10 รายการแรก
            this.filteredCustomersForDropdown = this.Show_List_MAS_Customer.slice(0, 10);
        } else {
            // ถ้ามี search term ให้ filter และแสดงผลลัพธ์ทั้งหมดที่ match
            const searchLower = this.customerSearchTerm.toLowerCase().trim();
            this.filteredCustomersForDropdown = this.Show_List_MAS_Customer.filter((customer: any) => {
                const name = (customer.CustomerNameTh || '').toLowerCase();
                return name.includes(searchLower);
            });
        }
    }

    // เมื่อมีการ search ใน dropdown
    onCustomerSearch(event: any): void {
        const searchTerm = event?.term || event || '';
    ...
    loadDataFromApi(): void {
        ...
        getData.subscribe((response: any) => {
            ...
            this.Show_List_MAS_Customer = response.Show_List_MAS_Customer;
            this.List_MAS_Customer = response.Show_List_MAS_Customer;
            this.updateFilteredCustomers();
    ...
    searchPayOrder(): void {
        ...
        let Send_r = {
            ...
            'CUSTOMER_CODE': this.customerId == '' ? 0 : this.customerId,
        }
        let model = {
            PROCESS_CODE: '',
            FUNC_CODE: 'FUNC-GET_DATA-SEARCH_SALEDASH',
        ...
        getData.subscribe((response: any) => {
            ...
            // Group ข้อมูลตาม ProductCode และ CustomerId
            // this.prepareDrugCustomerData();
...
getKpis(historyData: any[], stockData: any[], productCode: string): void {
  ...
  // สร้างกราฟแท่งปริมาณและมูลค่า
  prepareQuantityValueBarChart(): void {
    if (!this.Group_Pay_Drug || this.Group_Pay_Drug.length === 0) {
      this.quantityBarChartOptions = null;
      this.valueBarChartOptions = null;
      return;
    }

    // ตรวจสอบว่ามีการเลือกสถานพยาบาลหรือไม่
    const hasSelectedCustomer = this.customerId && this.customerId !== '';

    let aggregatedData: any[] = [];

    if (hasSelectedCustomer) {
      // ถ้าเลือกสถานพยาบาล: แกน x = ชื่อยา
      const groupedByProduct: Record<string, { quantity: number; value: number; productName: string }> = {};
      
      this.Group_Pay_Drug.forEach((item: any) => {
        const productCode = item.ProductCode || '';
        const productName = this.LIST_MAS_DRUG.find((d: any) => d.ProductCode === productCode)?.ProductTradeName || 'ไม่ระบุ';
        const quantity = parseFloat(item.PurchaseOrderItemApprovedAmount || '0');
        const price = parseFloat(this.LIST_MAS_DRUG.find((d: any) => d.ProductCode === productCode)?.ProductPrice || '0');
        const value = quantity * price;

        if (!groupedByProduct[productCode]) {
          groupedByProduct[productCode] = {
            quantity: 0,
            value: 0,
            productName: productName
          };
        }
        groupedByProduct[productCode].quantity += quantity;
        groupedByProduct[productCode].value += value;
      });

      aggregatedData = Object.values(groupedByProduct)
        .sort((a, b) => b.quantity - a.quantity)
        .slice(0, 20); // แสดง 20 รายการแรก
    } else {
      // ถ้าไม่ได้เลือกสถานพยาบาล: แกน x = สถานพยาบาล
      const groupedByCustomer: Record<string, { quantity: number; value: number; customerName: string }> = {};
      
      this.Group_Pay_Drug.forEach((item: any) => {
        // ลองหาค่า customerId จากหลายฟิลด์ที่เป็นไปได้
        const customerId = item.CustomerId || item.CustomerCode || item.Customer_ID || item.DataCode || '';
        
        // หาข้อมูล customer จาก List_MAS_Customer
        let customer = null;
        if (customerId) {
          customer = this.List_MAS_Customer.find((c: any) => 
        ...
        const customerName = customer?.CustomerNameTh || customer?.CustomerName || customer?.Name || 'ไม่ระบุ';
        const quantity = parseFloat(item.PurchaseOrderItemApprovedAmount || '0');
        const productCode = item.ProductCode || '';
        ...
        // ใช้ customerId หรือ customerName เป็น key
        ...
        groupedByCustomer[key].quantity += quantity;
        groupedByCustomer[key].value += value;
      });

      aggregatedData = Object.values(groupedByCustomer)
        .sort((a, b) => b.quantity - a.quantity)
        .slice(0, 20); // แสดง 20 รายการแรก
    }

    // สร้าง categories และ data
    const categories = aggregatedData.map(d => {
      const name = hasSelectedCustomer ? d.productName : d.customerName;
      return name.length > 25 ? name.substring(0, 25) + '...' : name;
    });
    const quantityData = aggregatedData.map(d => d.quantity);
    const valueData = aggregatedData.map(d => d.value);
    ...
    // กราฟมูลค่า
    const maxValue = Math.max(...valueData, 1);
  ...
  // Group ข้อมูลตาม ProductCode และ CustomerId สำหรับตาราง
  prepareDrugCustomerData(): void {
    if (!this.Group_Pay_Drug || this.Group_Pay_Drug.length === 0) {
      this.servicedrugCustomer.setGridData([]);
      return;
    }
    ...
    this.Group_Pay_Drug.forEach((item: any) => {
      const productCode = item.ProductCode || '';
      const customerId = item.CustomerId || item.CustomerCode || item.Customer_ID || item.DataCode || '';
      const key = `${productCode}_${customerId}`;
      ...
      if (!groupedData[key]) {
        groupedData[key] = {
          ...
          productName: drug.ProductTradeName || 'ไม่ระบุ',
          customerId: customerId,
          customerNameTh: item.CustomerNameTh || customer.CustomerNameTh || customer.CustomerName || 'ไม่ระบุ',
          customerNameEn: item.CustomerNameEn || customer.CustomerNameEn || '',
          ...
          rop: 0,
          avgDailyUse: 0,
          pricePerUnit: parseFloat(drug.ProductPrice || '0'),
          totalValue: 0,
          unit: drug.ProductUnitName || ''
        };
      }

      // รวมปริมาณ
      groupedData[key].quantity += parseFloat(item.PurchaseOrderItemApprovedAmount || '0');
    });

    // คำนวณค่า SOH, DOH, ROP, avgDailyUse สำหรับแต่ละ group
    Object.keys(groupedData).forEach(key => {
      const group = groupedData[key];
      const productCode = group.productCode;
      const customerId = group.customerId;

      // ดึงข้อมูลประวัติการซื้อของสินค้านี้และ customer นี้
      const hist = this.Group_Pay_Drug.filter((x: any) => {
        const xCustomerId = x.CustomerId || x.CustomerCode || x.Customer_ID || x.DataCode || '';
        return x.ProductCode === productCode && xCustomerId === customerId;
      });

      // ดึงข้อมูลสต็อกของสินค้านี้
      const stock = (this.List_product || []).filter((x: any) => x.ProductCode === productCode);
      ...
      // จำนวนที่ใช้ต่อวัน (avgDailyUse)
      const daily = avgMonthly > 0 ? avgMonthly / 30 : 0;

      // DOH (Days on Hand)
      const doh = daily > 0 ? (soh / daily) : 0;

      // มูลค่า (totalValue) = SOH * ราคาต่อหน่วย
      const totalValue = soh * group.pricePerUnit;

      // อัพเดทค่า
      group.soh = soh;
      group.doh = doh;
      group.rop = rop;
      group.avgDailyUse = daily;
      group.totalValue = totalValue;
    });

    // แปลงเป็น array และเรียงตาม productName
    ...
    // Map ข้อมูลให้ตรงกับโครงสร้างที่ตารางต้องการ
    const mappedData = dataArray.map(item => ({
      ...
      CustomerNameTh: item.customerNameTh,
      CustomerNameEn: item.customerNameEn,
      CustomerName: item.customerName,
      unit: item.unit
    }));

    this.servicedrugCustomer.setGridData(mappedData);
  }
