...
export class InventoryComponent {
    ...
    // ======= Period Lists =======
    listPeriod = [
        { IDA: 1, Period_Name: 'รายเดือน' },
        { IDA: 2, Period_Name: 'รายไตรมาส' },
        { IDA: 3, Period_Name: 'ช่วงวันที่' }
    ];
...
getKpis(historyData: any[], stockData: any[], productCode: string): void {
  ...
  fullModal(smallDataModal: any, data: any): void {
   ...
   prepareForecastChart(historyData: any[], stockData: any[], productCode: string): void {
    ...
    // 2) รวมยอดรายเดือนต่อสินค้าเดียวกัน (กันเคสมีหลายใบสั่ง/หลายแถวในเดือนเดียว)
    ...
    const monthly = Object.values(aggMap).sort((a, b) => a.label.localeCompare(b.label));
  
    // 3) 6 เดือนล่าสุดของสินค้านี้
    const last6 = monthly.slice(-6);
    const pastLabels = last6.map(m => m.label);
    const pastValues = last6.map(m => m.value);
  
    // กันข้อมูลว่าง
    const n = last6.length || 1;
    const avgMonthly = last6.length
      ? pastValues.reduce((a, b) => a + b, 0) / last6.length
      : 0;
    ...
    // 5) SD รายเดือนจาก last6
    const varianceMonth = last6.length
      ? pastValues.map(v => (v - avgMonthly) ** 2).reduce((a, b) => a + b, 0) / n
      : 0;
    const sdMonth = Math.sqrt(varianceMonth);
  
    // 6) Safety Stock (หน่วยเป็น “ชิ้น”) : z * σ * √L
    const safetyStock = z * sdMonth * Math.sqrt(Math.max(leadTimeMonths, 0));
  
    // 7) ROP (หน่วย “ชิ้น”) = demand ระหว่าง LT + safety stock
    //    ใช้หน่วยเดือนให้สอดคล้องกัน
    const ropUnits = (avgMonthly * leadTimeMonths) + safetyStock;
  
    // 8) DOH (วัน) และ stock คงเหลือรวม
    const stockQty = stock.reduce((a, b) => a + (Number(b.inspection_qty) || 0), 0);
    const daily = avgMonthly > 0 ? avgMonthly / 30 : 0;
    const doh = daily > 0 ? (stockQty / daily) : 0;
  
    // 9) สร้าง Forecast 3 เดือนถัดไปแบบ simple moving average
    const futureCount = 3;
    const lastLabel = pastLabels[pastLabels.length - 1] || this.getCurrentYYYYMM(); // fallback
    const forecastLabels: string[] = [];
    for (let i = 1; i <= futureCount; i++) {
      forecastLabels.push(this.addMonth(lastLabel, i));
    }
    const forecastValues: number[] = [];
    // เดือนแรกใช้ค่าจริงของเดือนสุดท้าย แล้วค่อยเฉลี่ยแบบง่าย (รักษา logic เดิมที่ต้องการ)
    if (last6.length) {
    //   forecastValues.push(pastValues[pastValues.length - 1]);
      forecastValues.push(avgMonthly);
    } else {
      forecastValues.push(avgMonthly);
    }
    for (let i = 2; i <= futureCount; i++) {
      const avgF = last6.length
        ? pastValues.reduce((a, b) => a + b, 0) / (last6.length + (i - 1))
        : avgMonthly;
  ...
  exportToExcel(): void {
    try {
      ...
      const data = this.griddataMasDrug.map((item: any, index: number) => ({
        'ลำดับ': index + 1,
        'ชื่อการค้า': item.ProductTradeName || '',
        'ประเภทยา': item.ProductTypeName || '',
        'กลุ่มยา': item.NarcoticGroupName || '',
        'ROP': item.rop || 0,
        'DOP': item.dop || 0,
        'จำนวนยา': item.soh || 0,
        'จำนวนที่ใช้ต่อวัน': item.daily_use || 0,
        'ราคาต่อหน่วย': item.pricePerUnit || 0,
        'ราคารวมต่อหน่วย': item.totalPricePerUnit || 0,
        'หน่วย': item.ProductUnitName || ''
      }));
      ...
      const columnWidths = [
        ...
        { wch: 12 },  // DOP
        { wch: 12 },  // จำนวนยา
        { wch: 15 },  // จำนวนที่ใช้ต่อวัน
        { wch: 15 },  // ราคาต่อหน่วย
        { wch: 18 },  // ราคารวมต่อหน่วย
        { wch: 10 }   // หน่วย
      ];
      ...
      // สร้างชื่อไฟล์
      const fileName = `รายการยา_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  exportToPDF(): void {
    try {
      ...
      const data = this.griddataMasDrug;
      let htmlContent = `
        <!DOCTYPE html>
        <html>
        <head>
      ...
      data.forEach((item: any, index: number) => {
        htmlContent += `
          <tr>
            ...
            <td>${item.ProductTradeName || ''}</td>
            <td>${item.ProductTypeName || ''}</td>
            <td>${item.NarcoticGroupName || ''}</td>
            <td style="text-align: right;">${(item.rop || 0).toFixed(2)}</td>
            <td style="text-align: right;">${(item.dop || 0).toFixed(2)}</td>
            <td style="text-align: right;">${(item.soh || 0).toFixed(2)}</td>
            <td style="text-align: right;">${(item.daily_use || 0).toFixed(2)}</td>
            <td style="text-align: right;">${(item.pricePerUnit || 0).toFixed(2)}</td>
            <td style="text-align: right;">${(item.totalPricePerUnit || 0).toFixed(2)}</td>
      ...
      if (printWindow) {
        ...
        setTimeout(() => {
          ...
          Swal.fire({
            icon: 'success',
            title: 'สำเร็จ',
            text: 'หน้าต่างพิมพ์ถูกเปิดแล้ว กรุณาเลือก "Save as PDF" ในหน้าต่างพิมพ์',
            timer: 3000
          });
        }, 250);
      } else {
        // ถ้าเปิดหน้าต่างใหม่ไม่ได้ ให้ใช้วิธี download HTML
        const link = document.createElement('a');
        link.href = 'data:text/html;charset=utf-8,' + encodeURIComponent(htmlContent);
        link.download = `รายการยา_${new Date().toISOString().split('T')[0]}.html`;
        link.click();
        
        Swal.fire({
          icon: 'info',
          title: 'ข้อมูลพร้อมพิมพ์',
          text: 'ไฟล์ HTML ถูกดาวน์โหลดแล้ว กรุณาเปิดไฟล์และใช้ Print to PDF',
          timer: 3000
        });
      }
    } catch (error) {
      ...
      Swal.fire({
        icon: 'error',
        title: 'เกิดข้อผิดพลาด',
        text: 'ไม่สามารถส่งออกข้อมูลเป็น PDF ได้'
      });
    }
  }
